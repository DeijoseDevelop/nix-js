  ...
  **With `invalidate`:**
  ...
  ```typescript
  import { NixComponent, createQuery, invalidateQueries, html, repeat } from "@deijose/nix-js";

  // --- In any component ---

  class ReservationsTable extends NixComponent {
    // createQuery returns reactive signals (status, data, error, etc.)
  ...
  **`QueryOptions`:**
  ...
  | `refetchOnMount` | `"always"` \| `boolean` | `true` | Controls background refetch on mount |
  ...
  **Class lifecycle:**
  ...
  | `createQuery(key, asyncFn, opts?)` | Key-based async fetching with built-in caching and global invalidation (hook) |
